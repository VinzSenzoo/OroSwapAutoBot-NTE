...
function loadSeedPhrases() {
  try {
    const data = fs.readFileSync("seed.txt", "utf8");
    seedPhrases = data.split("\n").map(phrase => phrase.trim()).filter(phrase => phrase.split(" ").length >= 12);
    if (seedPhrases.length === 0) throw new Error("No valid seed phrases in seed.txt");
    addLog(`Loaded ${seedPhrases.length} seed phrases from seed.txt`, "success");
  } catch (error) {
